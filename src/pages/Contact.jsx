...
export default function Contact() {
  ...
  return (
    <div className="py-6 sm:py-8">
      {/* WIDE HERO */}
      <section className="mx-auto w-[94%] sm:w-[92%] max-w-screen-2xl">
        <div className="relative overflow-hidden rounded-2xl ring-1 ring-black/5">
          {/* Key mobile changes:
              - shorter hero height on small screens
              - more compact padding
              - buttons become full-width on mobile */}
          <img
            src="/images/contact-us.png"
            alt="T Beauty Lounge contact"
            className="h-[40vh] w-full object-cover sm:h-[46vh] md:h-[60vh]"
            loading="lazy"
          />
          <div className="absolute inset-0 bg-brand-cream/70" />

          <div className="absolute inset-0 flex items-center">
            <div className="mx-auto w-11/12 max-w-3xl py-6 sm:py-10">
              <p className="text-xs sm:text-sm tracking-[0.22em] text-brand-forest/70">
                Contact Us
              </p>

              <h1 className="mt-2 text-3xl sm:text-4xl md:text-5xl font-semibold leading-tight text-brand-forest">
                We’re Here to Help
              </h1>
              ...
              <div className="mt-5 sm:mt-6 grid gap-3 sm:flex sm:flex-wrap">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full sm:w-auto inline-flex items-center justify-center rounded-full bg-brand-gold px-6 sm:px-7 py-3 text-white font-semibold shadow-sm hover:brightness-95"
                >
                  Book an Appointment
                </a>
                <a
                  href="tel:6232138996"
                  className="w-full sm:w-auto inline-flex items-center justify-center rounded-full border border-brand-gold bg-white/55 px-6 sm:px-7 py-3 text-brand-forest font-semibold hover:bg-brand-cream/70"
                >
                  Call us
                </a>
              </div>
      ...
      <main className="mx-auto w-[92%] max-w-7xl space-y-8 py-6 md:py-8">
        {/* ===== INFO CARDS ===== */}
        {/* Mobile change: 1 column by default, 2 columns at sm (so it doesn’t feel cramped) */}
        ...
        {/* ===== FORM + MAP ===== */}
        {/* Mobile change: stack with FORM first, MAP second; on lg make map a bit wider */}
        <section className="mt-10 grid gap-6 lg:grid-cols-[1fr_1.05fr]">
          {/* FORM */}
          <div className="rounded-[--radius-card] bg-white p-5 sm:p-6 md:p-8 shadow-sm ring-1 ring-black/5">
            ...
            <div className="mt-5 sm:mt-6 grid gap-4">
              <Field label="Name">
                <input
                  ...
                  inputMode="text"
                  autoComplete="name"
                  className="w-full rounded-xl bg-brand-cream/40 px-4 py-3.5 text-sm text-brand-forest outline-none ring-1 ring-black/10 focus:ring-2 focus:ring-brand-forest/40"
                  placeholder="Your name"
                />
              </Field>

              {/* Mobile change: 1 col on phones, 2 cols at sm */}
              ...
              {/* Mobile change: full-width primary button */}
              <a
                href={canSubmit ? mailtoHref : undefined}
                onClick={(e) => {
                  if (!canSubmit) e.preventDefault();
                }}
                className={`mt-1 inline-flex w-full justify-center rounded-full px-6 py-3.5 text-sm font-medium transition ${
                  canSubmit
              ...
              {!canSubmit && (
                <p className="text-xs text-brand-forest/70">
                  Please enter your name, a message, and agree to text consent.
                  Add phone or email so we can reply.
                </p>
              )}
            </div>

            {/* Social */}
            {/* Mobile change: wrap nicely + slightly bigger tap targets */}
            <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-brand-forest/85">
          ...
          <div className="rounded-[--radius-card] overflow-hidden bg-white shadow-sm ring-1 ring-black/5">
            ...
            {/* Mobile change: make map a friendlier height (not super tall),
                but still responsive. On larger screens it fills the column. */}
            ...
            {/* Optional: quick action row for mobile (super handy) */}
            <div className="grid grid-cols-2 gap-2 p-3 sm:hidden border-t border-black/5">
              <a
                href="https://www.google.com/maps/search/?api=1&query=1375+N+Litchfield+Rd+Suite+103+Goodyear+AZ+85395"
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center rounded-xl bg-brand-cream/70 px-3 py-2.5 text-sm font-medium text-brand-forest ring-1 ring-black/5"
              >
                Directions
              </a>
              <a
                href="tel:6232138996"
                className="inline-flex items-center justify-center rounded-xl bg-white px-3 py-2.5 text-sm font-medium text-brand-forest ring-1 ring-black/10"
              >
                Call
              </a>
            </div>
